...
// =========================================================================
// 🚀 IN-APP PREVIEW BOTTOM SHEET (Estilo Trivago / Google Flights)
// =========================================================================
function openInAppProductSheet(event, url, farmacia, nombre, precio) {
  event.preventDefault();
  
  let modal = document.getElementById('in-app-sheet-modal');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'in-app-sheet-modal';
    modal.innerHTML = `
      <div class="inapp-backdrop" onclick="closeInAppSheet()"></div>
      <div class="inapp-sheet">
        <div class="inapp-handle"></div>
        <div class="inapp-header">
          <div class="inapp-pharm-badge" id="sheet-pharm-badge">🏥 Farmacia Cruz Verde</div>
          <button class="inapp-close-btn" onclick="closeInAppSheet()">✕</button>
        </div>
        <div class="inapp-body">
          <div class="inapp-icon-box">💊</div>
          <h3 id="sheet-prod-name" class="inapp-prod-name">Paracetamol 500 mg 16 comprimidos</h3>
          <div class="inapp-price-box">
            <span class="inapp-price-label">Precio en tienda:</span>
            <span id="sheet-prod-price" class="inapp-prod-price">$990</span>
          </div>
          <div class="inapp-guarantee">
            <span>🛡️ <strong>Compra 100% Segura</strong> en el sitio oficial de la farmacia</span>
          </div>
        </div>
        <div class="inapp-actions">
          <a id="sheet-action-btn" href="#" target="_blank" class="inapp-btn-primary" onclick="handleOutboundClick()">
            <span>🛒 Ir a Comprar a la Farmacia</span>
            <span class="inapp-btn-arrow">↗</span>
          </a>
          <button class="inapp-btn-secondary" onclick="closeInAppSheet()">
            ← Seguir comparando en QueFarmacia
          </button>
        </div>
      </div>
    `;
  ...
  document.getElementById('sheet-pharm-badge').textContent = '🏥 ' + farmacia;
  document.getElementById('sheet-prod-name').textContent = nombre;
  document.getElementById('sheet-prod-price').textContent = precio;
  
  const actionBtn = document.getElementById('sheet-action-btn');
  actionBtn.href = url;
  actionBtn.innerHTML = `<span>🛒 Ir a Comprar a ${farmacia}</span><span class="inapp-btn-arrow">↗</span>`;
  
  modal.classList.add('inapp-active');
  document.body.style.overflow = 'hidden';
}

function closeInAppSheet() {
  const modal = document.getElementById('in-app-sheet-modal');
  if (modal) modal.classList.remove('inapp-active');
